...
beforeAll(async () => {
    ...
    pathExplorationService = new PathExplorationService(queryService, SPARQL_EP, embeddingHelper);
}, 30000);
...
async function testBasicPathExploration() {
    ...
    try {
        const result = await pathExplorationService.explore(sourceUri, targetUri, "business relationships", 10, 2);
        ...
        expect(result).toContain('[dbo:board]');
        expect(result).toContain('↳ dbr:Apple_Inc.');
...
async function testShortPath() {
    ...
    try {
        const result = await pathExplorationService.explore(sourceUri, targetUri, "occupation", 100, 2);
